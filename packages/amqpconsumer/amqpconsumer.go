...
func (c *Consumer) AnnounceQueue(queueName, bindingKey string) (<-chan amqp.Delivery, error) {
	...
	log.Printf("declaring dead letter Queue %q", bindingKey)
	_, err = c.channel.QueueDeclare(
		bindingKey, // name of the queue
		true,       // durable
		false,      // delete when usused
		false,      // exclusive
		false,      // noWait
	...
	if err != nil {
		return nil, fmt.Errorf("Dead letter queue Declare: %s", err)
	}
